...
const two = inventors.map((inventor) => { return `${inventor.first} ${inventor.last}` });
console.log(two);
...
const seven=people.sort((first,second)=>{
    const [ff,ll]=first.split(', ');
    const [sf,sl]=second.split(', ');
    return ll>sl?1:-1;
})
console.log(seven);
// 8. Reduce Exercise
